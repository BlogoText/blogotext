...
class Parsedown {
	function text($text) {
		...
		$text = trim($text, "\n");
		$lines = explode("\n", $text); // split in lines
		$markup = $this->lines($lines); // read thru lines
		$markup = trim($markup, "\n");
		return $markup;
	}
	...
	private function lines(array $lines) {
		...
		foreach ($Blocks as $Block) {
			if (isset($Block['hidden'])) continue;
			$markup .= "\n";
	...
	protected function blockCode($Line, $Block = null) {
		if (isset($Block) and ! isset($Block['type']) and ! isset($Block['interrupted'])) return;
	...
	protected function blockComment($Line) {
		if ($this->markupEscaped) return;
		if (isset($Line['text'][3]) and $Line['text'][3] === '-' and $Line['text'][2] === '-' and $Line['text'][1] === '!') {
			$Block = array('markup' => $Line['body'],);
			if (preg_match('/-->$/', $Line['text'])) $Block['closed'] = true;
			return $Block;
		}
	}
	...
	protected function blockHeader($Line) {
		if (isset($Line['text'][1])) {
			$level = 1;
			while (isset($Line['text'][$level]) and $Line['text'][$level] === '#') $level++;
			if ($level > 6) return;
	...
	protected function blockSetextHeader($Line, array $Block = null) {
		if ( ! isset($Block) or isset($Block['type']) or isset($Block['interrupted'])) return;
	...
	protected function blockMarkup($Line) {
		...
		if (preg_match('/^<(\w*)(?:[ ]*'.$this->regexHtmlAttribute.')*[ ]*(\/)?>/', $Line['text'], $matches)) {
			...
			if (trim($remainder) === '') {
				...
				if (isset($matches[2]) or in_array($matches[1], $this->voidElements)) return;

				if (preg_match('/<\/'.$matches[1].'>[ ]*$/i', $remainder)) $Block['closed'] = true;
			}
			return $Block;
		}
	}
	...
	protected function inlineMarkup($Excerpt)	{
		if ($this->markupEscaped or strpos($Excerpt['text'], '>') === false) return;
